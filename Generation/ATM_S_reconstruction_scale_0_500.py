...
class Proj_eeg(nn.Sequential):
    def __init__(self, embedding_dim=840, proj_dim=1024, drop_proj=0.5):
        super().__init__(
...
def train_model(eegmodel, imgmodel, dataloader, optimizer, device, text_features_all, img_features_all):
    ...
    for batch_idx, (eeg_data, labels, text, text_features, img, img_features) in enumerate(dataloader):
        ...
        optimizer.zero_grad()
        eeg_features = eegmodel(eeg_data[:, :, :125]).float()
...
def main():
    config = {
        ...
        "logger": True,
        "insubject": True,
        "encoder_type": 'ATM_S_reconstruction_scale_0_500',
    ...
    for sub in subjects:
        ...
        eeg_model = globals()[config['encoder_type']](63, int(250*0.5))
