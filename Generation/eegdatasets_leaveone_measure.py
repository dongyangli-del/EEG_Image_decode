...
class EEGDataset():
    ...
    def __init__(self, data_path, exclude_subject=None, subjects=None, train=True, time_window=[0, 1.0], classes = None, pictures = None):
        ...
        generated_features_filename = os.path.join(f'./embedding/{sub}/{model_type}_features_train.pt') if train else os.path.join(f'./embedding/{sub}/{model_type}_features_test.pt')
        # if os.path.exists(features_filename) :
        generated_saved_features = torch.load(generated_features_filename)
        saved_features = torch.load(features_filename)
        self.text_features = saved_features['text_features']
        self.img_features = saved_features['img_features']
        self.generated_features = generated_saved_features['img_features']
            # else:
