...
class ATM_S_insubject_retrieval_Temporal(nn.Module):    
    def __init__(self, num_channels=10, sequence_length=250, num_subjects=1, num_features=64, num_latents=1024, num_blocks=1):
        super(ATM_S_insubject_retrieval_Temporal, self).__init__()
...
def main():  
    config = {
        ...
        "logger": True,
        "insubject": True,
        "encoder_type": 'ATM_S_insubject_retrieval_Temporal',
    ...
    for sub in subjects:                    
        ...
        train_dataset = EEGDataset("Temporal", data_path, subjects=[sub] if config['insubject'] else [], exclude_subject=sub if not config['insubject'] else None, train=True)
        test_dataset = EEGDataset("Temporal", data_path, subjects=[sub] if config['insubject'] else [], exclude_subject=sub if not config['insubject'] else None, train=False)
