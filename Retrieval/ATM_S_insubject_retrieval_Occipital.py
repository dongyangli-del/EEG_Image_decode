...
def main():
    ...
    if config['insubject']:
        ...
        for sub in subjects:                    
            train_dataset = EEGDataset("Occipital", data_path, subjects=[sub], train=True)
            test_dataset = EEGDataset("Occipital",  data_path, subjects=[sub], train=False)
        ...
        for sub in subjects:
            train_dataset = EEGDataset("Occipital", data_path, exclude_subject=sub, train=True)
            test_dataset = EEGDataset("Occipital", data_path, exclude_subject=sub, train=False)
            train_loader = DataLoader(train_dataset, batch_size=config['batch_size'], shuffle=True, num_workers=0, drop_last=True)
            test_loader = DataLoader(test_dataset, batch_size=1, shuffle=True, num_workers=0, drop_last=True)
