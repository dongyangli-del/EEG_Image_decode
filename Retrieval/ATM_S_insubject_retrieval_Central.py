...
def main():  
    config = {
        ...
        "logger": True,
        "insubject": True,
        "encoder_type": 'ATM_S_insubject_retrieval_Central',
    ...
    for sub in subjects:                    
        ...
        train_dataset = EEGDataset("Central", data_path, subjects=[sub] if config['insubject'] else [], exclude_subject=sub if not config['insubject'] else None, train=True)
        test_dataset = EEGDataset("Central", data_path, subjects=[sub] if config['insubject'] else [], exclude_subject=sub if not config['insubject'] else None, train=False)
