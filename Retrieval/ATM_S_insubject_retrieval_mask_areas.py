...
class ATMS(nn.Module):    
    def __init__(self, num_channels=63, sequence_length=250, num_subjects=1, num_features=64, num_latents=1024, num_blocks=1):
        ...
        self.masking = DynamicMasking(whole_brain_proportion=0.3)
    ...
    def forward(self, x):
        x, mask = self.masking(x, self.ch_names)
...
class DynamicMasking(nn.Module):
    def __init__(self, whole_brain_proportion=0.3):
        super(DynamicMasking, self).__init__()
        self.whole_brain_proportion = whole_brain_proportion  
    ...
    def forward(self, x, ch_names):
        ...
        use_whole_brain = random.random() < self.whole_brain_proportion
        mask = None
        if not use_whole_brain:
            
            selected_region = random.choice(list(self.electrode_regions.keys()))
            electrodes = self.electrode_regions[selected_region]
            indices = [ch_names.index(elec) for elec in electrodes if elec in ch_names]
    ...
    for epoch in range(config['epochs']):
        ...
        if (epoch +1) % 5 == 0:                    
            ...
            if config['insubject']==True:       
                os.makedirs(f"./models/masked_areas/{config['encoder_type']}/{sub}/{current_time}", exist_ok=True)             
                file_path = f"./models/masked_areas/{config['encoder_type']}/{sub}/{current_time}/{epoch+1}.pth"
                torch.save(eeg_model.state_dict(), file_path)            
            else:                
                os.makedirs(f"./models/masked_areas/across/{config['encoder_type']}/{current_time}", exist_ok=True)             
                file_path = f"./models/masked_areas/across/{config['encoder_type']}/{current_time}/{epoch+1}.pth"
                torch.save(eeg_model.state_dict(), file_path)
