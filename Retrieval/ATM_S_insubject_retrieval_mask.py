    ...
    for epoch in range(config['epochs']):
        ...
        if (epoch +1) % 5 == 0:                    
            ...
            if config['insubject']==True:       
                os.makedirs(f"./models/masked/{config['encoder_type']}/{sub}/{current_time}", exist_ok=True)             
                file_path = f"./models/masked/{config['encoder_type']}/{sub}/{current_time}/{epoch+1}.pth"
                torch.save(eeg_model.state_dict(), file_path)            
            else:                
                os.makedirs(f"./models/masked/across/{config['encoder_type']}/{current_time}", exist_ok=True)             
                file_path = f"./models/masked/across/{config['encoder_type']}/{current_time}/{epoch+1}.pth"
                torch.save(eeg_model.state_dict(), file_path)
