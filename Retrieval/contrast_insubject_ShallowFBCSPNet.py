...
def main():  
    ...
    "logger": True,
    "insubject":True,
    "encoder_type":'ShallowFBCSPNet_Encoder',
    ...
    if config['insubject']:
        for sub in subjects:                    
            ...
            results_file = f"./outputs/contrast/{config['encoder_type']}/{sub}/{current_time}/{(config['encoder_type'])}_{sub}.csv"            
        ...
        for sub in subjects:
            train_dataset = EEGDataset(data_path, exclude_subject=sub, subjects=subjects, train=True)
            test_dataset = EEGDataset(data_path, exclude_subject=sub, subjects=subjects, train=False)
            train_loader = DataLoader(train_dataset, batch_size=config['batch_size'], shuffle=True, num_workers=0, drop_last=True)
            test_loader = DataLoader(test_dataset, batch_size=1, shuffle=True, num_workers=0, drop_last=True)
            ...
            # Save results to a CSV file
            os.makedirs(f"./outputs/contrast/{config['encoder_type']}/{sub}/{current_time}", exist_ok=True)          
            results_file = f"./outputs/contrast/{config['encoder_type']}/{sub}/{current_time}/{config['encoder_type']}_cross_exclude_{sub}.csv"            
            with open(results_file, 'w', newline='') as file:
                writer = csv.DictWriter(file, fieldnames=results[0].keys())
                writer.writeheader()
                writer.writerows(results)
...
'''
1.位置编码的EEGEncoder比其他EEGEncoder 要好  0.5%~1.5%->2%
2.图像+文本两项loss，并且引入超参数均衡两项loss，效果会好 2%->3%
3.训练集batch_size越小测试集精度越好，越大越差且难以收敛 2%->3%，测试集batch_size最好为1
4.温度参数τ越小越好，测试集精度刚开始迅速上升
5.优化器使用Adam测试集精度上升快
'''