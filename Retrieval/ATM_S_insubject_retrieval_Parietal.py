...
class Proj_eeg(nn.Sequential):
    def __init__(self, embedding_dim=4320, proj_dim=1024, drop_proj=0.5):
        super().__init__(
...
class ATM_S_insubject_retrieval_Parietal(nn.Module):    
    def __init__(self, num_channels=16, sequence_length=250, num_subjects=1, num_features=64, num_latents=1024, num_blocks=1):
        super(ATM_S_insubject_retrieval_Parietal, self).__init__()
...
def main():  
    config = {
        ...
        "logger": True,
        "insubject": True,
        "encoder_type": 'ATM_S_insubject_retrieval_Parietal',
    ...
    for sub in subjects:                    
        ...
        train_dataset = EEGDataset("Parietal", data_path, subjects=[sub] if config['insubject'] else [], exclude_subject=sub if not config['insubject'] else None, train=True)
        test_dataset = EEGDataset("Parietal", data_path, subjects=[sub] if config['insubject'] else [], exclude_subject=sub if not config['insubject'] else None, train=False)
        ...
        results_dir = f"./outputs/{config['encoder_type']}/{sub}/{current_time}"
